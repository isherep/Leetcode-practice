...
public class FindFruitCombs {
    ...
    public static int winPrize(String[][] codeList, String[] shoppingCart) {
        ...
        // and if th cart index plus codelist.length below the shop
        while (i < codeList.length && j + codeList[i].length <= shoppingCart.length) {
            boolean match = true;
            // iterating in the each group
            for (int k = 0; k < codeList[i].length; k++) {
                String listItem = codeList[i][k];
                String cartItem = shoppingCart[j + k];
                System.out.println("list item: " + listItem + ", cart item: " + cartItem);
            ...
            if (match) {
                // move the codelist pointer so that it's start with next codelist
                j += codeList[i].length;
                i++;
            } else {
                j++;
